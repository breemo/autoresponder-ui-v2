...
export default function AdminPlans() {
  ...
  const fetchPlans = async () => {
    ...
    const { data, error } = await supabase.from("plans").select("id, name, price, description, allow_self_edit, auto_replies_limit, ai_replies_limit, integrations_limit, messages_limit").order("price", { ascending: true });
  ...
  const handleSubmit = async (e) => {
	...
	const payload = {
	  ...
	  auto_replies_limit:
		form.auto_replies_limit === ""
		  ? null
		  : Number(form.auto_replies_limit),
	  integrations_limit:
		form.integrations_limit === ""
		  ? null
		  : Number(form.integrations_limit),
	  ai_replies_limit:
	    form.ai_replies_limit === ""
		  ? null
		  : Number(form.ai_replies_limit),
	  messages_limit:
		form.messages_limit === ""
		  ? null
		  : Number(form.messages_limit)
	};
  ...
  setForm({
	  ...
	  ai_replies_limit: plan.ai_replies_limit ?? "",
  ...
  return (
    <div className="space-y-5" dir="rtl">
      ...
      {loading ? <div className="rounded-3xl bg-white p-10 text-center text-slate-500">جارِ تحميل الباقات...</div> : <div className="grid grid-cols-1 gap-5 xl:grid-cols-3">{plans.map((p) => <div key={p.id} className={`${cardClass} overflow-hidden`}><div className="border-b border-slate-100 p-6"><div className="flex items-start justify-between gap-3"><div><h3 className="text-xl font-black text-slate-950">{p.name}</h3><p className="mt-1 text-sm text-slate-500">{p.description || "بدون وصف"}</p></div><span className="rounded-2xl bg-indigo-50 px-4 py-2 text-xl font-black text-indigo-700">${p.price || 0}</span></div></div>
	  
	  <div className="space-y-3 p-6 text-sm">
	  
	  <div className="flex justify-between">
	  <span className="text-slate-500">حد الردود</span>
	  <b>{p.auto_replies_limit ?? "—"}</b>
	  </div>
	  ...
	  <div className="flex justify-between">
	  <span className="text-slate-500">حد التكاملات</span>
	  <b>{p.integrations_limit ?? "—"}</b>
	  </div>
	  
	  <div className="flex justify-between">
	  <span className="text-slate-500">حد الرسائل</span>
	  <b>{p.messages_limit ?? "—"}</b>
	  </div>
	  
	  <div className="flex justify-between">
	  <span className="text-slate-500">تعديل الميزات</span>
	  <span className={`rounded-full px-3 py-1 text-xs font-bold ${p.allow_self_edit ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{p.allow_self_edit ? "مسموح" : "ممنوع"}</span>
	  </div>
	  
	  </div>
	  
	  <div className="flex gap-2 border-t border-slate-100 p-4"><Link to={`/admin/plan-features/${p.id}`} className="flex-1 rounded-2xl bg-indigo-50 px-3 py-2 text-center text-xs font-bold text-indigo-700 hover:bg-indigo-100">ميزات الباقة</Link><button onClick={() => startEdit(p)} className="rounded-2xl border border-slate-200 px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50">تعديل</button><button onClick={() => deletePlan(p.id)} className="rounded-2xl bg-red-50 px-3 py-2 text-xs font-bold text-red-600 hover:bg-red-100">حذف</button></div></div>)}</div>}
	  ...
	  <div>
	  <label className="mb-1 block text-sm font-bold">حد الردود</label>
	  <input type="number" className={inputClass} name="auto_replies_limit" value={form.auto_replies_limit} onChange={handleChange} />
	  </div>
		...
		<div>
		  ...
		  <input
			...
			name="ai_replies_limit"
			value={form.ai_replies_limit}
			onChange={handleChange}
		  />
		</div>

	  <div>
	  <label className="mb-1 block text-sm font-bold">حد التكاملات</label>
	  <input type="number" className={inputClass} name="integrations_limit" value={form.integrations_limit} onChange={handleChange} />
	  </div>
	  </div>
