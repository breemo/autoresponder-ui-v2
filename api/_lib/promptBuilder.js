...
function buildBehaviorSection(aiBehavior) {
  ...
  const lines = [
    ...
    "Once you have answered the current request, end the reply. Do NOT add a closing courtesy line, a standing offer of further help, or an invitation to ask more — in any wording and any language (\"إذا كان لديك أي استفسار آخر\", \"لا تتردد في التواصل\", \"Let me know if…\", \"feel free to…\", etc.) — unless the current request genuinely requires a concrete next step.",
    "If the customer's whole message is just an acknowledgement or thanks, reply with a brief acknowledgement only — nothing else, and do not re-explain the previous answer.",
...
function buildRulesSection(client, aiBehavior) {
  ...
  const lines = [
    ...
    "- UNKNOWN: when the provided context is simply silent on something (neither confirmed nor denied), never phrase that silence as a negative fact — do not say or imply \"we don't have X\" when the truth is only that it isn't confirmed in what you have. Instead, briefly and naturally say that specific detail isn't confirmed, still in the business voice, then STOP. If part of what they actually asked IS known, answer just that part. Do not pad the reply with unrelated content and do not add \"let me know\", \"feel free to ask\", \"I can check with the team\" or any similar offer by default. Offer to involve a teammate ONLY when the customer needs that specific unresolved detail or action and there is genuinely no way for you to answer it — never as a sign-off. Vary the wording naturally; do not fall back on one fixed disclaimer sentence. Follow the Escalation instructions above if any are given.",
...
function buildKnowledgeSection(relevantKnowledge) {
  ...
  return [
    ...
    "Time-sensitive facts (a promotion, campaign, temporary price, schedule, availability window, event): if an excerpt describes one but does not establish that it is valid right now (no current dates or validity stated), you may state what is on file, but say clearly that you cannot confirm it is currently active. Never flatly deny that it exists when it is in an excerpt, and never claim it is active without evidence.",
    "",
    excerpts,
  ].join("\n");
}
